...
router.post('/',verfifyToken,async(req,res)=>{
    ...
    if(decoded){
        ...
        try{
            ...
            return res.status(200).json({success:true,msg:`List saved successfully`})
        }
        catch(err){
            return res.status(500).json({success:false,msg:`Internal servor error`})
        }
    }
   else{
    return res.status(401).json({success:false,msg:`decoded details not found`})
   }
})
